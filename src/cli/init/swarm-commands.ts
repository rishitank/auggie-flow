...
export async function createSwarmCommands(): Promise<void> {
  ...
  const swarmDir = '.auggie/commands/swarm';
...
## Usage
\`\`\`bash
auggie-flow swarm "Research objective" --strategy research --mode distributed --parallel
\`\`\`
...
## Usage
\`\`\`bash
auggie-flow swarm "Build application" --strategy development --mode hierarchical --parallel
\`\`\`
...
## Usage
\`\`\`bash
auggie-flow swarm "Analyze data" --strategy analysis --parallel --max-agents 10
\`\`\`
...
## Usage
\`\`\`bash
auggie-flow swarm "Test application" --strategy testing --mode mesh --parallel
\`\`\`
...
## Usage
\`\`\`bash
auggie-flow swarm "Optimize performance" --strategy optimization --mode hybrid --parallel
\`\`\`
...
## Usage
\`\`\`bash
auggie-flow swarm "System maintenance" --strategy maintenance --mode centralized --monitor
\`\`\`
...
auggie-flow swarm "Research modern web frameworks" --strategy research --mode distributed --parallel --max-agents 6

# Market analysis with coordinated batch operations
auggie-flow swarm "Analyze AI market trends" --strategy research --parallel --monitor --timeout 120
\`\`\`

### Development Tasks with Batch Coordination
\`\`\`bash
# Hierarchical development with batch file operations
auggie-flow swarm "Build microservice API" --strategy development --mode hierarchical --parallel --max-agents 8

# React dashboard with coordinated component development
auggie-flow swarm "Create React dashboard" --strategy development --parallel --monitor --output sqlite
\`\`\`

### Analysis Tasks with Batch Processing
\`\`\`bash
# Mesh-coordinated data analysis
auggie-flow swarm "Analyze user behavior data" --strategy analysis --mode mesh --parallel --max-agents 10

# Performance analysis with monitoring
auggie-flow swarm "Performance analysis of application" --strategy analysis --monitor --output csv
\`\`\`

### Testing Tasks with Parallel Validation
\`\`\`bash
# Comprehensive parallel testing
auggie-flow swarm "Comprehensive testing suite" --strategy testing --parallel --max-agents 12

# Security testing with distributed coordination
auggie-flow swarm "Security testing analysis" --strategy testing --mode distributed --monitor
\`\`\`

### Optimization Tasks with Hybrid Coordination
\`\`\`bash
# Database optimization with hybrid approach
auggie-flow swarm "Optimize database queries" --strategy optimization --mode hybrid --parallel

# Frontend optimization with batch processing
auggie-flow swarm "Frontend performance optimization" --strategy optimization --monitor --max-agents 6
\`\`\`

### Maintenance Tasks with Centralized Control
\`\`\`bash
# Dependency updates with centralized coordination
auggie-flow swarm "Update dependencies safely" --strategy maintenance --mode centralized --monitor

# System health checks with batch operations
auggie-flow swarm "System health check" --strategy maintenance --parallel --output json
\`\`\`
