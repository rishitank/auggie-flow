...
async function main() {
  ...
  if (
    ...
    if (detailedHelp && !detailedHelp.includes('Help not available')) {
      ...
      console.log('\nRun "auggie-flow --help" to see available commands.');
    }
    return;
  }
  ...
  if (hasCommand(command)) {
    try {
      ...
      console.log(`\nRun "auggie-flow ${command} --help" for usage information.`);
      return;
    }
  }
...
`);
    ...
    config: async (key) => {
      try {
        const config = JSON.parse(await readTextFile('auggie-flow.config.json'));
        if (key) {
  ...
  function updatePrompt() {
    rl.setPrompt(
      replState.currentSession ? `auggie-flow:${replState.currentSession}> ` : 'auggie-flow> ',
    );
  }
