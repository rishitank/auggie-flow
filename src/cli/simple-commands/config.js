...
async function initConfig(subArgs, flags) {
  ...
  const configFile = 'auggie-flow.config.json';
  ...
  try {
    ...
    console.log('\nNext steps:');
    console.log('1. Review settings: auggie-flow config show');
    console.log('2. Customize values: auggie-flow config set <key> <value>');
    console.log('3. Validate config: auggie-flow config validate');
  } catch (err) {
...
async function showConfig(subArgs, flags) {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      console.log(`   Path: ${config.memory?.path || './memory/auggie-flow-data.json'}`);
      console.log('\n🤖 Agents:');
    ...
    console.log('Run "auggie-flow config init" to create default configuration');
  }
}
...
async function getConfigValue(subArgs, flags) {
  ...
  if (!key) {
    ...
    console.log('  auggie-flow config get terminal.poolSize');
    console.log('  auggie-flow config get orchestrator.maxConcurrentTasks');
    return;
  }
  ...
  try {
    ...
    printError('Configuration file not found');
    console.log('Run "auggie-flow config init" to create configuration');
  }
}
...
async function setConfigValue(subArgs, flags) {
  ...
  if (!key || value === undefined) {
    ...
    console.log('Examples:');
    console.log('  auggie-flow config set terminal.poolSize 15');
    console.log('  auggie-flow config set orchestrator.taskTimeout 600000');
    return;
  }
...
async function validateConfig(subArgs, flags) {
  ...
  try {
    ...
    console.log('Run "auggie-flow config init" to create valid configuration');
  }
}
...
function showConfigHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  auggie-flow config init');
  console.log('  auggie-flow config set terminal.poolSize 15');
  console.log('  auggie-flow config get orchestrator.maxConcurrentTasks');
  console.log('  auggie-flow config validate');
}
