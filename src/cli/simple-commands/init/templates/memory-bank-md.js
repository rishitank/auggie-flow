...
- **Primary**: JSON database (\`./memory/auggie-flow-data.json\`) with parallel access
...
### Batchtools Commands
- \`npx auggie-flow memory batch-store <entries-file>\`: Store multiple entries in parallel
- \`npx auggie-flow memory parallel-query <queries-file>\`: Execute multiple queries concurrently
- \`npx auggie-flow memory concurrent-export <namespaces>\`: Export multiple namespaces simultaneously
- \`npx auggie-flow memory batch-cleanup <retention-config>\`: Clean up multiple namespaces in parallel

## Configuration (Enhanced)
Memory settings are configured in \`auggie-flow.config.json\` with batchtools optimizations:
\`\`\`json
{
  "memory": {
    "backend": "json",
...
npx auggie-flow memory batch-store sparc-data.json --namespace sparc --parallel

# Concurrent query across multiple namespaces
npx auggie-flow memory parallel-query "authentication design" --namespaces arch,impl,test

# Batch export with parallel compression
npx auggie-flow memory concurrent-export project-backup --compress --parallel
\`\`\`

### Performance Monitoring
\`\`\`bash
# Monitor concurrent operations
npx auggie-flow memory stats --concurrent --verbose

# Analyze batch operation performance
npx auggie-flow memory performance-report --batchtools

# Check parallel indexing status
npx auggie-flow memory index-status --parallel
\`\`\`
...
npx auggie-flow memory debug --concurrent

# Analyze batch operation performance
npx auggie-flow memory analyze --batchtools --verbose

# Validate parallel index integrity
npx auggie-flow memory index-validate --parallel --repair
\`\`\`
