...
auggie-flow init --enhanced --sparc

# Enhanced with specific options
auggie-flow init --safe --sparc --force

# Validation only (no initialization)
auggie-flow init --validate-only

# Skip specific validations if needed
auggie-flow init --enhanced --skip-pre-validation --skip-backup
```
...
auggie-flow init --validate

# Skip specific validation phases
auggie-flow init --validate --skip-pre-init --skip-config --skip-mode-test

# Pre-initialization check only
auggie-flow init --validate --pre-init-only
```
...
auggie-flow init --rollback --full

# Partial rollback for specific phase
auggie-flow init --rollback --partial --phase sparc-init

# Interactive rollback (shows available points)
auggie-flow init --rollback

# List available backups and checkpoints
auggie-flow init --list-backups
```
...
- **claude-commands**: Remove Auggie Code slash commands
...
- **Storage**: Backups stored in `.auggie-flow-backups/`
...
4. **Partial Initialization**
   ...
   auggie-flow init --enhanced --sparc --force
   # Or start fresh
   auggie-flow init --rollback --full
   ```
...
2. **Clean State Reset**
   ...
   rm -rf memory/ coordination/ auggie-flow
   ```
