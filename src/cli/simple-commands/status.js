...
function displayStatus(status, verbose) {
  ...
  if (!status.orchestrator.running) {
    console.log('   Run "auggie-flow start" to begin orchestration');
  }
  if (status.agents.active === 0) {
    console.log('   Run "auggie-flow agent spawn researcher" to create an agent');
  }
  if (status.memory.entries === 0) {
    console.log('   Run "auggie-flow memory store key value" to test memory');
  }
}
