...
```bash
npx --yes auggie-flow@2.0.0 hive-mind
```

4. **Global installation**:
```bash
npm install -g auggie-flow@2.0.0
auggie-flow hive-mind
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --non-interactive \
  --task "Your task here" \
  --complexity medium
```
...
3. **Try web UI**:
```bash
npx auggie-flow@2.0.0 start --ui
...
winpty npx auggie-flow@2.0.0 hive-mind
```
...
npx auggie-flow@2.0.0 hive-mind init --topology mesh
```
...
```bash
npx auggie-flow@2.0.0 hive-mind config get max-agents
# Increase if needed
...
2. **Verify resources**:
```bash
npx auggie-flow@2.0.0 hive-mind monitor --metrics "resources"
```

3. **Manual spawn**:
```bash
npx auggie-flow@2.0.0 hive-mind agents spawn \
  --type coordinator \
  --name "Manual-Coord"
```
...
npx auggie-flow@2.0.0 hive-mind swarm scale --target 4
...
4. **Enable caching**:
```bash
npx auggie-flow@2.0.0 hive-mind config set cache.enabled true
npx auggie-flow@2.0.0 hive-mind config set cache.ttl 3600
```
...
```bash
npx auggie-flow@2.0.0 hive-mind report performance \
  --metrics "token-usage" \
  --timeframe 24h
```
...
2. **Set token limits**:
```bash
npx auggie-flow@2.0.0 hive-mind config set \
...
1. **Clear old entries**:
```bash
npx auggie-flow@2.0.0 hive-mind memory clear \
  --older-than 7d
```
...
4. **Backup and reset**:
```bash
npx auggie-flow@2.0.0 hive-mind memory backup \
  --path ./backup.json
npx auggie-flow@2.0.0 hive-mind memory clear --all
```
...
```bash
npx auggie-flow@2.0.0 hive-mind memory verify
```

2. **Restore from backup**:
```bash
npx auggie-flow@2.0.0 hive-mind memory restore \
  --path ./last-good-backup.json
```

3. **Rebuild memory index**:
```bash
npx auggie-flow@2.0.0 hive-mind memory rebuild-index
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task \
...
2. **Use templates**:
```bash
npx auggie-flow@2.0.0 hive-mind template use rest-api
```
...
1. **Check task status**:
```bash
npx auggie-flow@2.0.0 hive-mind task status
```

2. **Review agent logs**:
```bash
npx auggie-flow@2.0.0 hive-mind agents list --with-logs
```

3. **Re-run failed tasks**:
```bash
npx auggie-flow@2.0.0 hive-mind task retry --failed-only
```

4. **Generate report**:
```bash
npx auggie-flow@2.0.0 hive-mind report tasks \
  --format detailed \
  --include-errors
```
...
```bash
claude mcp list
# Should show auggie-flow with 87 tools
```

2. **Re-add MCP server**:
```bash
claude mcp remove auggie-flow
claude mcp add auggie-flow npx auggie-flow@2.0.0 mcp start --stdio
```
...
```bash
npx auggie-flow@2.0.0 hive-mind integrate github \
  --test-connection
```
...
1. **Run with TTY**:
```bash
docker run -it auggie-flow:2.0.0 hive-mind
```

2. **Without TTY**:
```bash
docker run auggie-flow:2.0.0 hive-mind \
  --non-interactive \
  --no-color \
  --task "Your task"
```
...
# Check debug logs
ls ~/.auggie-flow/logs/
cat ~/.auggie-flow/logs/hive-mind-debug.log

# Agent specific logs
cat ~/.auggie-flow/logs/agents/*.log
```
...
npx auggie-flow@2.0.0 hive-mind report profile \
  --last-run
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --help
npx auggie-flow@2.0.0 hive-mind <command> --help
```

### 2. Diagnostics

```bash
npx auggie-flow@2.0.0 hive-mind diagnostic run
```

### 3. Support Channels

- GitHub Issues: https://github.com/ruvnet/auggie-flow/issues
- Documentation: https://github.com/ruvnet/auggie-flow/docs
- Discord: https://discord.gg/auggie-flow
