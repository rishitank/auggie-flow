...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Create CRUD API for books with title, author, ISBN" --complexity low
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Build todo app with React frontend, Node.js backend, user accounts, and PostgreSQL" --complexity high
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Split monolithic Node.js API into user, product, and order microservices with API gateway"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Create GraphQL API for social media with users, posts, comments, and real-time subscriptions"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Create serverless image processing API with AWS Lambda, S3, and DynamoDB"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Migrate existing Express.js API to Fastify while maintaining all endpoints and functionality"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Optimize React app performance - reduce bundle size, improve rendering, add code splitting"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Optimize PostgreSQL database - analyze slow queries, add indexes, optimize schema"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Build real-time analytics dashboard with WebSockets, data visualization, and historical data"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Create CI/CD pipeline with GitHub Actions for Node.js app with testing, linting, and deployment"
```
...
```bash
npx auggie-flow@2.0.0 hive-mind --task "Generate complete documentation for existing codebase including API docs, guides, and examples"
```
...
npx auggie-flow@2.0.0 hive-mind --template rest-api --task "Extend with GraphQL support"
```
...
npx auggie-flow@2.0.0 hive-mind --task "Create Express API" --export base-api.json

# Then enhance it
npx auggie-flow@2.0.0 hive-mind --task "Add authentication to existing API" --import base-api.json
```
...
const { HiveMind } = require('auggie-flow');
...
# Check bottlenecks
npx auggie-flow@2.0.0 hive-mind analyze bottlenecks
```
