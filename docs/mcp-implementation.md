...
### 7. Claude-Flow Tools (`src/mcp/auggie-flow-tools.ts`)
...
```bash
# Via CLI
auggie-flow mcp-call agents/spawn '{"type": "researcher", "name": "Research Assistant"}'
...
```bash
# Start MCP server
auggie-flow start --mcp-transport stdio

# Client connection
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{...}}' | auggie-flow mcp
```
...
```bash
# Start HTTP server
auggie-flow start --mcp-transport http --port 3000
...
auggie-flow start --log-level debug --mcp-transport http
```
...
1. **Add new tools** in `src/mcp/auggie-flow-tools.ts`
