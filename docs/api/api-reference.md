# 📖 Auggie Flow v2.0.0 API Reference
...
> **RECOMMENDED**: Use `npx auggie-flow@2.0.0` for all operations until local build issues are resolved.
...
npx auggie-flow@2.0.0 swarm "your objective" --max-agents 3
npx auggie-flow@2.0.0 sparc "research modern frameworks"

# Initialize project (creates .claude directory structure)
npx auggie-flow@2.0.0 init --sparc

# Enterprise swarm coordination
npx auggie-flow@2.0.0 swarm "build REST API" --strategy development --parallel
```
...
### `init`
Initialize Auggie Flow in your project.
...
# Minimal setup
./auggie-flow init --minimal

# CI/CD setup
./auggie-flow init --ci --sparc
```

### `start`
Start the Auggie Flow orchestration system.
...
# Custom ports
./auggie-flow start --port 8080 --mcp-port 8081

# Background mode
./auggie-flow start --detached
```
...
### `stop`
Stop the Auggie Flow system.

```bash
auggie-flow stop [options]
```
...
```bash
auggie-flow agent spawn <type> [name] [options]
```
...
npx auggie-flow@2.0.0 agent spawn coder "API Developer" --capabilities "rest-api,database,testing"
```
...
npx auggie-flow@2.0.0 swarm "Research cloud architecture patterns" --strategy research --mode hierarchical

# Development swarm with monitoring
npx auggie-flow@2.0.0 swarm "Implement user management system" --strategy development --parallel --monitor

# Large-scale enterprise swarm
npx auggie-flow@2.0.0 swarm "Optimize database performance" --max-agents 8 --mode mesh --ui
```
...
```bash
# Auto-mode selection
npx auggie-flow@2.0.0 sparc "design authentication system"

# Specific mode usage
npx auggie-flow@2.0.0 sparc architect "design microservices architecture"
npx auggie-flow@2.0.0 sparc tdd "user registration feature"
npx auggie-flow@2.0.0 sparc security "audit API endpoints"

# List all modes
npx auggie-flow@2.0.0 sparc modes

# Get mode details
npx auggie-flow@2.0.0 sparc info security
```
...
```bash
auggie-flow memory store <key> <value> [options]
```
...
```bash
auggie-flow memory get <key> [options]
```
...
```bash
auggie-flow memory search <query> [options]
```
...
```bash
auggie-flow task create "<description>" [options]
```
...
```bash
auggie-flow github <mode> [options]
```
...
npx auggie-flow@2.0.0 github pr-manager "coordinate feature review"

# Automated issue tracking
npx auggie-flow@2.0.0 github issue-tracker "track bug resolution"

# Release coordination
npx auggie-flow@2.0.0 github release-manager "prepare v2.1.0 release"
```
...
```bash
npx auggie-flow swarm "test" --max-agents 2
...
**For Immediate Use**:
- ✅ Use `npx auggie-flow@2.0.0` for all operations
...
```bash
auggie-flow config <action> [key] [value]
```
...
```bash
# Set UI port
./auggie-flow config set ui.port 8080

# Get swarm settings
./auggie-flow config get swarm

# List all settings
./auggie-flow config list
```
...
# Create hierarchy
./auggie-flow agent spawn coordinator --name "Lead"
./auggie-flow agent spawn coder --parent lead-123
```
...
```bash
auggie-flow agent assign <agent-id> <task> [options]
```
...
```bash
auggie-flow swarm init [options]
```
...
```bash
# Hierarchical swarm
./auggie-flow swarm init --topology hierarchical --max-agents 10

# Mesh network
./auggie-flow swarm init --topology mesh --strategy adaptive
```
...
```bash
auggie-flow swarm <task> [options]
```
...
```bash
# Development task
./auggie-flow swarm "Build REST API with authentication" \
  --strategy development \
  --parallel \
  --monitor

# Research task
./auggie-flow swarm "Research best practices for microservices" \
  --strategy research \
  --max-agents 5
```
...
```bash
auggie-flow swarm status [options]
```
...
```bash
auggie-flow swarm monitor [options]
```
...
```bash
auggie-flow sparc run <mode> <prompt> [options]
```
...
```bash
# Architecture design
./auggie-flow sparc run architect "design microservices for e-commerce"

# TDD workflow
./auggie-flow sparc run tdd "user authentication system"

# Chained execution
./auggie-flow sparc run --chain spec,architect,code "payment processing"
```
...
```bash
auggie-flow sparc template <action> [name]
```
...
```bash
# Store text
./auggie-flow memory store api-design "RESTful endpoints for user management"

# Store JSON
./auggie-flow memory store config '{"api": {"version": "2.0"}}' --type json

# Store with tags
./auggie-flow memory store requirements "User auth with JWT" \
...
```bash
auggie-flow memory query <pattern> [options]
```
...
```bash
# Query by pattern
./auggie-flow memory query "auth"

# Query with filters
./auggie-flow memory query "*" --tags "security" --limit 10
```
...
```bash
auggie-flow memory delete <key> [options]
```
...
./auggie-flow task create development "Implement user authentication" \
  --priority high \
  --assign coder-123

# Create with dependencies
./auggie-flow task create testing "Integration tests" \
...
```bash
auggie-flow task update <task-id> [options]
```
...
```bash
auggie-flow task workflow <workflow-file> [options]
```
...
```bash
auggie-flow mcp status [options]
```
...
```bash
auggie-flow mcp call <tool> [params] [options]
```

**Examples:**
```bash
# Initialize swarm
./auggie-flow mcp call swarm_init \
  --params '{"topology": "mesh", "maxAgents": 5}'

# Store memory
./auggie-flow mcp call memory_usage \
...
```bash
auggie-flow github pr-manager <description> [options]
```
...
```bash
auggie-flow github issue-solver <issue> [options]
```
...
```bash
auggie-flow github release-coordinator <version> [options]
```
...
```bash
auggie-flow github sync-packages [options]
```
...
```bash
auggie-flow github repo-architect [action] [options]
```
...
```bash
auggie-flow project <action> [name] [options]
```
...
```bash
auggie-flow deploy <action> [target] [options]
```
...
```bash
auggie-flow cloud <action> [resource] [options]
```
...
```bash
auggie-flow security <action> [target] [options]
```
...
```bash
auggie-flow analytics <action> [options]
```
...
```bash
auggie-flow logs [component] [options]
```
...
```bash
auggie-flow benchmark [test] [options]
```
...
# 2. Start system
./auggie-flow start --ui --monitor

# 3. Create swarm for development
./auggie-flow swarm init --topology hierarchical --max-agents 8

# 4. Execute development task
./auggie-flow swarm "Build complete REST API with authentication" \
  --strategy development \
  --parallel \
  --monitor
...
./auggie-flow github pr-manager "Feature: Complete REST API implementation"

# 7. Deploy
./auggie-flow deploy create v1.0.0 --strategy blue-green
```
