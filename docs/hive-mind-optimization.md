...
auggie-flow hive-mind-optimize --clean-memory --memory-days 30
```
...
auggie-flow hive-mind-optimize --archive-tasks --task-days 7
```
...
auggie-flow hive-mind-optimize --vacuum
```
...
0 2 * * 0 /usr/local/bin/auggie-flow hive-mind-optimize --auto --clean-memory --archive-tasks
```
...
- name: Optimize Hive Mind Database
  run: |
    auggie-flow hive-mind-optimize --auto --report
...
For more information, see the [Hive Mind Documentation](./hive-mind.md) or run `auggie-flow hive-mind-optimize --help`.