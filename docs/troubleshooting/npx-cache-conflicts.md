...
When running multiple `auggie-flow` instances concurrently, you may encounter the following error:
...
NPX uses a shared cache directory (`/tmp/.npm/_npx/` or similar) to store downloaded packages. When multiple `auggie-flow` processes run concurrently (e.g., in swarm mode or parallel batch operations), they compete for the same cache resources, causing directory rename conflicts.
...
1. **Unique Cache Directories**: Each process gets its own NPX cache at `/tmp/.npm-cache/auggie-flow-[pid]-[timestamp]-[random]`
...
1. **Use isolated cache directories**:
   ```bash
   NPM_CONFIG_CACHE=/tmp/auggie-flow-cache-$$ npx --y auggie-flow@alpha init --force
   ```

2. **Install globally to avoid NPX**:
   ```bash
   npm install -g auggie-flow@alpha
   auggie-flow init --force
   ```

3. **Run operations sequentially**:
   ```bash
   # In batch operations, disable parallelism
   auggie-flow batch init --parallel=false
   ```
