...
### Basic Swarm
```bash
auggie-flow swarm "Build a complete e-commerce platform with React and Node.js"
```

### Research-Focused Swarm
```bash
auggie-flow swarm "Research and document best practices for microservices architecture" \
  --strategy research \
  --research \
  --max-agents 8
```
...
```bash
auggie-flow swarm "Refactor the authentication system with modern security practices" \
...
```bash
auggie-flow swarm "Migrate monolithic application to microservices" \
  --coordinator \
  --max-depth 4 \
  --timeout 120 \
  --review
...
### Dry Run to Preview
```bash
auggie-flow swarm "Create mobile app" --dry-run --verbose
```
...
```bash
auggie-flow swarm "debug task" --verbose --dry-run
```
...
```bash
auggie-flow swarm "Create a full-stack task management app with React, Node.js, PostgreSQL" \
  --strategy development \
  --max-agents 8 \
  --parallel \
  --review \
...
### Research Project
```bash
auggie-flow swarm "Research and compare top 10 JavaScript frameworks for enterprise applications" \
  --strategy research \
  --research \
  --max-agents 10 \
...
### Code Migration
```bash
auggie-flow swarm "Migrate legacy jQuery application to modern React with TypeScript" \
  --strategy development \
  --review \
  --max-depth 4 \
  --timeout 180
