# Migration Guide: Auggie Flow v1 to v2.0.0

## Overview

Auggie Flow v2.0.0 introduces significant improvements to environment handling, particularly for non-interactive environments like VS Code integrated terminal, CI/CD systems, and Docker containers. This guide will help you migrate from v1 to v2 smoothly.
...
```json
{
  "label": "Initialize Auggie Flow",
  "type": "shell",
  "command": "npx auggie-flow init --dangerously-skip-permissions",
  "problemMatcher": []
}
```
...
```json
{
  "label": "Initialize Auggie Flow",
  "type": "shell",
  "command": "npx auggie-flow@2.0.0 init",
  "problemMatcher": [],
  "presentation": {
    "reveal": "always",
    "panel": "dedicated"
...
**v1 GitHub Actions:**
```yaml
- name: Run Auggie Flow
  run: |
    npx auggie-flow init --dangerously-skip-permissions
    npx auggie-flow swarm "build project" --dangerously-skip-permissions
```
...
- name: Run Auggie Flow
  run: |
    ...
    npx auggie-flow@2.0.0 swarm "build project"
```
...
**v1 Dockerfile:**
```dockerfile
RUN npx auggie-flow init --dangerously-skip-permissions --force
```
...
{
  "scripts": {
    "claude:init": "auggie-flow init --dangerously-skip-permissions",
    "claude:swarm": "auggie-flow swarm --dangerously-skip-permissions"
  }
}
```
...
{
  "scripts": {
    "claude:init": "auggie-flow init",
    "claude:swarm": "auggie-flow swarm",
    "claude:env-check": "auggie-flow env-check",
    "claude:init:force": "auggie-flow init --non-interactive"
  }
}
```
...
1. **Check Environment Detection:**
   ```bash
   npx auggie-flow@2.0.0 env-check
   ```

2. **Test in Your Environment:**
   ```bash
   # VS Code Terminal
   npx auggie-flow@2.0.0 init --dry-run
   
   # CI/CD (simulate)
   CI=true npx auggie-flow@2.0.0 init --dry-run
   
   # Docker (simulate)
   DOCKER_CONTAINER=true npx auggie-flow@2.0.0 init --dry-run
   ```
...
3. **Verify Prompt Defaults:**
   ...
   npx auggie-flow@2.0.0 init --non-interactive
   ```
...
auggie-flow init --dangerously-skip-permissions
```
...
auggie-flow init --non-interactive
```
...
Create `~/.auggie-flow/prompt-defaults.json`:
```json
{
  "global": [
    {
...
```bash
# Development
NODE_ENV=development npx auggie-flow@2.0.0 init

# Production (more conservative defaults)
NODE_ENV=production npx auggie-flow@2.0.0 init

# Custom environment
CLAUDE_ENVIRONMENT=staging npx auggie-flow@2.0.0 init
```
...
4. **Use Specific Flags When Needed:**
   ...
   CI=true npx auggie-flow@2.0.0 init --interactive
...
```bash
# Downgrade
npm install -g auggie-flow@1.0.0

# Add back manual flags
export CLAUDE_V1_MODE=1
alias auggie-flow='auggie-flow --dangerously-skip-permissions'
```
...
2. **Enable Debug Mode:**
   ```bash
   DEBUG=auggie-flow:* npx auggie-flow@2.0.0 init
   ```
...
Auggie Flow v2.0.0 makes working in non-interactive environments much easier:
